...
CoAP-to-CoAP proxy MAY respond to requests with an Echo option, but MUST remove the Echo option it recognizes as one generated by itself on follow-up requests. However, it MUST relay the Echo option of responses unmodified, and MUST relay the Echo option of requests it does not recognize as generated by itself unmodified. A CoAP-to-CoAP proxy MAY however add an Echo Option to a response that does not have an Echo Option.
